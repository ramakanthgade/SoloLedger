...
describe('countBillableUnits', () => {
  it('counts one unit per in-FY disposal (a swap already expands to one sell)', () => {
    ...
    expect(countBillableUnits(disposals, [], FY, JUR, { ...TEST_TAX_SETTINGS, jurisdiction: JUR, reportingCurrency: 'INR' })).toBe(2);
  });
  ...
  it('excludes internal transfers / dust / fees (they never produce a disposal)', () => {
    ...
    expect(countBillableUnits([], transfers, FY, JUR, { ...TEST_TAX_SETTINGS, jurisdiction: JUR, reportingCurrency: 'INR' })).toBe(0);
  });

  it('counts policy-supported income and excludes review-required gifts', () => {
    const income = [
    ...
    expect(countBillableUnits([], income, FY, JUR, { ...TEST_TAX_SETTINGS, jurisdiction: JUR, reportingCurrency: 'INR' })).toBe(2);
  });

  it('sums in-FY disposals + income events', () => {
    const disposals = [disposal('a', IN_FY)];
    const income = [incomeTx('i1', IN_FY, 'staking')];
    expect(countBillableUnits(disposals, income, FY, JUR, { ...TEST_TAX_SETTINGS, jurisdiction: JUR, reportingCurrency: 'INR' })).toBe(2);
  });
