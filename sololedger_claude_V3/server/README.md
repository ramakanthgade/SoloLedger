...
Probes every supported connector through the tunnel — tier 2 checks public endpoint reachability and response shape; tier 3 sends browser-shaped, format-valid dummy-key auth requests and asserts exact observed exchange-origin status/code/message tuples. Bitstamp, Bitget, MEXC, BitMart, and Bitvavo reject an unknown key before validating the dummy signature (and BitMart memo), so those probes establish exact host/path/header reachability, not signature integrity. Gemini's dummy-key result likewise proves only that the request reached Gemini's authenticated endpoint; it does not validate a real account key, secret, role, signature, or historical-data access. Byte-exact signed header/body forwarding is covered by `src/routes/exchangeTunnel.test.ts`. Exits non-zero on any failure.
