...
- Bitvavo is pinned to `api.bitvavo.com`: only GET requests to `/v2/time`, `/v2/markets`, `/v2/balance`, `/v2/trades`, `/v2/depositHistory`, and `/v2/withdrawalHistory` are accepted. Only the four `bitvavo-access-*` signing headers are forwarded; order and withdrawal mutation paths are unreachable.
